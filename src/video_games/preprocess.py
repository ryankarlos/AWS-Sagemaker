...
def visualise_target_imbalance(df):
    ...
    viz["User_Score"] = pd.Series(
        viz["User_Score"].apply(pd.to_numeric, errors="coerce")
    )
    viz["User_Score"] = viz["User_Score"].mask(
        np.isnan(viz["User_Score"]), viz["Critic_Score"] / 10.0
    )
...
def upload_model_data_to_s3():
    upload_file(
        ...
        S3_BUCKET,
        f"{S3_PREFIX_VIDEOGAMES}/train/train.csv",
    )
    upload_file(
        os.path.join(data_dir, "validation.csv"),
        S3_BUCKET,
        f"{S3_PREFIX_VIDEOGAMES}/validation/validation.csv",
    )
    upload_file(
        os.path.join(data_dir, "test.csv"),
        S3_BUCKET,
        f"{S3_PREFIX_VIDEOGAMES}/test/test.csv",
    )
